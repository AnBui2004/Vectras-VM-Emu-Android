...
public class ImeHeightProvider {
    ...
    public static void assistActivity(Activity activity) {
        ...
        sensor.getViewTreeObserver().addOnGlobalLayoutListener(() -> {
            ...
            X11Activity.getRealMetrics(metrics);
    ...
    private static void report(Activity activity, FrameLayout content, int bottom, WindowInsetsCompat insets) {
        ...
        X11Activity.getInstance().setCaptionHeight((insets != null && !child.getFitsSystemWindows())
                ? insets.getInsets(WindowInsetsCompat.Type.captionBar()).top : 0);

        int imeHeight = 0;
        if (activity.hasWindowFocus() && !SamsungDexUtils.checkDeXEnabled(activity) && (SDK_INT > VERSION_CODES.N && !activity.isInPictureInPictureMode())) {
            int navBottom = (insets != null && child.getFitsSystemWindows())
                    ? insets.getInsets(WindowInsetsCompat.Type.navigationBars()).bottom : 0;
            imeHeight = Math.max(0, bottom - navBottom);
        }
        X11Activity.getInstance().setImeHeight(imeHeight);
    }
}